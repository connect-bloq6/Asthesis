...
import { BackCaseLayer, BatteryLayer, MotherboardLayer } from '@/components/canvas/models/layers'
...
function AnimatedLayers({ scrollProgress }: AnimatedLayersProps) {
  ...
  const motherboardRef = useRef<Group>(null)
  ...
  useFrame(() => {
    ...
    if (motherboardRef.current) {
      // Motherboard moves with battery (slightly ahead)
      const motherboardZ = 0.1 + scrollProgress * 1.5
      motherboardRef.current.position.z = motherboardZ
      
      // Slight rotation as it comes out
      motherboardRef.current.rotation.x = scrollProgress * 0.1
      motherboardRef.current.rotation.y = scrollProgress * 0.15
    }
  })
  ...
  return (
    <group rotation={[0.3, 0.5, 0]}>
      ...
      {/* Motherboard - animates with battery, positioned over it */}
      <group ref={motherboardRef} position={[0, 0, 0.1]}>
        <MotherboardLayer 
          position={[0, 0, 0]} 
          rotation={[0, 0, 0]} 
          scale={0.95} 
